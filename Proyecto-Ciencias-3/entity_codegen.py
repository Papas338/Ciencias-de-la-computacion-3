...
def main(debug=False):
    ...
    # Build restaurant model from restaurante.ent file
    restaurant_model = entity_mm.model_from_file(join(this_folder, 'restaurante.ent'))

    def is_entity(n):
        """
        Test to prove if some type is an entity
        """
        if n.type in restaurant_model.entities:
            return True
        else:
            return False
    ...
    for entity in restaurant_model.entities:
